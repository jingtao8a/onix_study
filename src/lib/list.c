...
// 链表插入排序,维护了一个按照task的ticks值得升序列表
void list_insert_sort(list_t *list, list_node_t *node, int offset) {
    // 从链表找到第一个比当前节点 key 点更大的节点，进行插入到前面
    list_node_t *anchor = &list->tail;
    int key = element_node_key(node, offset);//取出将要插入的task对应ticks
    for (list_node_t *ptr = list->head.next; ptr != &list->tail; ptr = ptr->next)
    {
        int compare = element_node_key(ptr, offset);//取出睡眠链表中task对应ticks
        if (compare > key)//找到第一个大于key的node
        {
            anchor = ptr;
            break;
        }
    ...
    // 插入链表
    list_insert_before(anchor, node);//插入进去
}
