...
{
    uint8 ei_magic[4];    // 内容为 0x7F, E, L, F
    uint8 ei_class;       // 文件种类 1-32位，2-64 位
    uint8 ei_data;        // 标记大小端 1-小端，2-大端
    uint8 ei_version;     // 与 e_version 一样，必须为 1
    uint8 ei_pad[16 - 7]; // 占满 16 个字节
} _packed ELFIdent;
...
typedef struct Elf32_Sym
{
    ...
    uint8 st_info;          // 指定符号类型和约束属性，见 SymbolBinding
    uint8 st_other;         // 为 0，无意义
...
static void load_segment(inode_t *inode, Elf32_Phdr *phdr) {
    ...
    // 需要页的数量
    uint32 count = div_round_up(MAX(phdr->p_memsz, phdr->p_filesz), PAGE_SIZE);
    for (size_t i = 0; i < count; i++) {
        uint32 addr = vaddr + i * PAGE_SIZE;
        assert(addr >= USER_EXEC_ADDR && addr < USER_MMAP_ADDR);
        link_page(addr);
    }
    inode_read(inode, (char *)vaddr, phdr->p_filesz, phdr->p_offset);
    if (phdr->p_filesz < phdr->p_memsz) {//如果该段在内存占用大小大于文件占用大小，末尾补0
        memset((char *)vaddr + phdr->p_filesz, 0, phdr->p_memsz - phdr->p_filesz);
    }
    // 如果段不可写，则置为只读
    if ((phdr->p_flags & PF_W) == 0) {
        for (size_t i = 0; i < count; i++) {//将该段对应的所有页都设置为readonly
            uint32 addr = vaddr + i * PAGE_SIZE;
...
static uint32 copy_argv_envp(char *filename, char *argv[], char *envp[]) {
    ...
    uint32 pages_end = pages + 4 * PAGE_SIZE;
...
int sys_execve(char *filename, char *argv[], char *envp[]) {
    ...
    intr_frame_t *iframe = (intr_frame_t *)((uint32)task + PAGE_SIZE - sizeof(intr_frame_t));

    iframe->edx = 0;//TODO 动态链接器的地址
    iframe->eip = entry;//指向程序入口地址
    iframe->esp = top;
