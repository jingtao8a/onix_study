...
interrupt_handler_%1:
    ; xchg bx, bx
%ifn %2
    push 0x20222202;当%2为0时压栈一个错误码(cpu不会帮忙压栈error code的情况)
%endif
...
interrupt_entry:
    ...
    mov eax, [esp + 12 * 4];//中断向量号

    push eax
    call [handler_table + eax * 4];//调用对应的中断处理函数

interrupt_exit:  
    add esp, 4;对应push eax, 调用结束恢复栈
    ...
    add esp, 8;对应push %1和之前压入的error code, 调用结束恢复栈
    iret
...
syscall_handler:
    ...
    add esp, (6 * 4); 弹出压入的六个个参数
    mov dword[esp + 8 * 4], eax;修改栈中的 eax寄存器, 设置系统调用的返回值
