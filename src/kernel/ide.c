...
static void ide_handler(int vector) {
    ...
    ide_ctrl_t *ctrl = &controllers[vector - IRQ_HARDDISK - 0x20];//根据中断号判断是哪个控制器通道发送了信号

    //读取常规状态寄存器，表示中断处理结束
    uint8 state = inb(ctrl->iobase + IDE_STATUS);
    LOGK("harddisk interrupt vectro %d state 0x%x\n", vector, state);
    if (ctrl->waiter) {
        task_unblock(ctrl->waiter);//唤醒该进程
        ctrl->waiter = NULL;
    }
}
...
int ide_pio_write(ide_disk_t *disk, void *buf, uint8 count, uint32 lba)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        task_t *task = running_task();
        if (task->state == TASK_RUNNING) {//阻塞自己等磁盘写数据完成
            ctrl->waiter = task;
            task_block(task, NULL, TASK_BLOCKED);
        }
        ide_busy_wait(ctrl, IDE_SR_NULL);
...
rollback:
    reentrant_unlock(&disk->ctrl->lock);
    return ret;
}

static void ide_part_init(ide_disk_t *disk, uint16 *buf) {
    if (!disk->total_lba) {//如果该磁盘不存在,直接返回
        return;
    }
    //读主引导扇区
    ide_pio_read(disk, buf, 1, 0);//tasks_init之前，磁盘读写仍然使用的是同步IO

    //初始化主引导扇区对象
    boot_sector_t *boot = (boot_sector_t *)buf;
    
    for (size_t i = 0; i< IDE_PART_NR; ++i) {
        part_entry_t *entry =  &boot->entry[i];//指向分区表项
        ide_part_t *part = &disk->parts[i];
        if (!entry->count) {//该分区不存在
            continue;//查看下一个分区表项
        }
        if (entry->system == PART_FS_EXTENDED) {//如果该分区是扩展分区类型
            continue;//不支持扩展分区
            // panic("Unsupported extended partition\n");
        }
...
static int ide_pio_ioctl(ide_disk_t *disk, int cmd, void *args, int flags) {
    switch (cmd) {
        ...
        default:
            panic("device command %d can not be recognized");
    }
}
...
void ide_init() {
    ...
    ide_install();//安装设备
    //注册硬盘中断处理函数，打开屏蔽字
    set_interrupt_handler(IRQ_HARDDISK, ide_handler);//通道1
    set_interrupt_handler(IRQ_HARDDISK2, ide_handler);//通道2
